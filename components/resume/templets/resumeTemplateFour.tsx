...
const ResumeTemplateFour = ({
  ...
  return (
    ...
    >
      <div className="bg-container text-on-container px-10 py-8 w-full max-w-[800px] mx-auto leading-snug">
        {/* HEADER */}
        <div>
          <h1 className="text-3xl font-bold tracking-wide">
            {profileInfo?.firstname} {profileInfo?.lastname}
          </h1>
          <div className="mt-2 flex flex-wrap gap-4 text-sm text-on-container">
        ...
        <SectionMain title="PROFILE SUMMARY" color={primaryColor}>
          {profileInfo?.description ? (
            <p className="text-sm text-on-container mt-2 text-justify leading-relaxed">
              {profileInfo.description}
            </p>
          ) : (
          ...
          {resumeData.experience?.length ? (
            resumeData.experience.map((exp: any, i: number) => (
              <div key={i} className="mt-1 text-sm">
                <p className="font-semibold text-on-container">
                ...
                <p className="italic text-xs text-on-muted">
                  {exp.jobType} • {exp.location}
                </p>
                <p className="mt-1 text-on-container leading-relaxed">
                  {exp.description}
                </p>
              </div>
            ))
          ...
          {resumeData.education?.length ? (
            resumeData.education.map((edu: any, i: number) => (
              <div key={i} className="mt-1 text-sm">
                ...
                {edu.location && (
                  <p className="italic text-xs text-on-muted">{edu.location}</p>
                )}
                {edu.description && (
                  <p className="mt-1 text-on-container leading-relaxed">
                    {edu.description}
          ...
          {resumeData.projects?.length ? (
            resumeData.projects.map((proj: any, i: number) => (
              <div key={i} className="mt-1 text-sm">
                ...
                <p className="text-xs text-on-muted">{proj.technologies}</p>
                <p className="mt-1 text-on-container leading-relaxed">
                  {proj.description}
                </p>
                {proj.link && (
          ...
          {resumeData.certifications?.length ? (
            resumeData.certifications.map((cert: any, i: number) => (
              <div key={i} className="mt-1 text-sm">
                ...
                <p className="italic text-xs text-on-muted">{cert.issuer}</p>
                <p className="text-xs text-on-muted">
        ...
        <SectionMain title="SKILLS & LANGUAGES" color={primaryColor}>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <ul className="list-disc list-inside text-xs text-on-container">
...
const Empty = ({ label }: any) => (
  <p className="text-xs italic text-on-muted mt-1">{label}</p>
);
